...
## 1. 设备概述
### 1.1 设备定义
RexForce 测力主板是一款用于测力采集的控制主板，可连接各类测力传感器，用于制造运动测力台、肌力测试仪、力量评估设备、康复评估设备、平衡测试平台等运动与康复类设备，以及工业测力计、拉压力测试机、吊装测力装置、工装夹具测力装置、智能称重终端等工业类设备。

### 1.2 功能说明
设备支持 **BLE 蓝牙** 连接和**额外串口模块**有线连接，支持手机、平板、带蓝牙功能的电脑等客户端接入，并以 **1000 Hz** 采样速率实时输出重量数据，适用于测力采集、数据显示与无线传输等应用场景。
...
### 5.2 有线连接
- 有线模式可用于主设备与副设备之间的数据传输，也支持连接串口模块直接连接电脑客户端。
...
## 8. 归零校准操作
制作好测力设备后，必须先使用实际使用场景重量的**精确砝码**进行校准。
...
9. **有线模式范围**：有线模式用于两台 RexForce 主板互联，也支持有线接串口模块连接电脑客户端。
...
### 11.1 示例程序
文档中提供的示例 Python、html（网页）、ios 程序可作为上位机接入参考，用于演示 RexForce 测力主板的 BLE 扫描、设备连接、数据接收、帧解析及指令发送等基础开发流程，建议用网页程序例程，功能较完善。

该程序可实现以下功能：

1. 扫描并连接 `Force-XXXX` 设备。运行程序前，应确保客户端设备具备 BLE 功能，且 RexForce 主板已正常启动状态；
...
### 11.3 开发者应用登记
欢迎用户基于 RexForce 测力主板，自行开发 App、微信小程序或其他相关应用。
...
[点此查看测力台采购组装教程](测力台采购装配/README.md)

<p align="center">
  <img src="./测力台采购装配/pic/安装.jpg" alt="RexForce 主板尺寸图" width="700" />
</p>
...
[【淘宝】MuscleForce肌力测力计 肌肉力量测试仪 专业运动康复评估](https://e.tb.cn/h.8jrn9edppDYU9H7?tk=XEAST14bC2i) 

<p align="center">
  <img src="./pic/测力计购买.jpg"  width="300" />
</p>
